...
cutoutaddon=1; % time in seconds added to template cutouts (at start and end) for warping
...
for y=1:size(peaks,1)
    ...
    if y>1  %this helps to avoid that several close crosscorr peaks within one motif cause many overlapping motif detections
    ...
    for e=1:size(motifpos,2)  %for each motif
        ...
        for f=1:size(Warping.templates,2) %for each warped template 
            ...
            slidingCrossCorr_warp = [];
                ...
                Crosscorr_maxpos=[Crosscorr_maxpos; warping_range(f)/1000 [maxcross maxposcross]]; %warping factor, crosscorr peak, position of crosscorr peak
